...
from image_misc import FormattedString, cv2_typeset_text, to_255, cv2_imshow_rgb
from input_fetcher import InputSignalFetcher
...
class SignalVis(object):
    ...
    def init_window(self):
        ...
        for pane_name, pane_dimensions in self.settings.window_panes:
            if len(pane_dimensions) != 4:
                raise ImproperlyConfigured(
                    'pane "{}" dimensions should be a tuple of length 4, but it is "{}"'.format(pane_name,
                                                                                                pane_dimensions))
            ...
            if pane_name in self.panes:
                raise Exception('Duplicate pane name in settings: {}'.format(pane_name))
        ...
        if self.debug_level == 3:
            print 'BUFFER IS: {} {} {}'.format(self.window_buffer.shape, self.window_buffer.min(),
                                               self.window_buffer.max())
    ...
    def run_loop(self):
        ...
        # Setup
        self.init_window()
        self.input_updater = InputSignalFetcher(self.settings)
        ...
        while not self.quit:
            ...
            for heartbeat in heartbeat_functions:
                if self.debug_level == 3:
                    print 'Heartbeat: calling {}'.format(heartbeat)
                heartbeat()
            ...
            if self.debug_level == 3:
                print 'Since last: {}'.format(now - last_render)
    ...
    def handle_key_pre_apps(self, key):
        ...
        if tag == 'static_file_increment':
            ...
            self.input_updater.set_debug(self.debug_level)
            for app_name, app in self.apps.iteritems():
    ...
    def draw_help(self):
        ...
        lines = []
        lines.append([FormattedString('~ ~ ~ Signal Deep Visualization Toolbox ~ ~ ~', defaults, align='center',
