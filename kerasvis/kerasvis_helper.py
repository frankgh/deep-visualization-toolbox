...
def get_pretty_layer_name(settings, layer_name):
    has_old_settings = hasattr(settings, 'kerasvis_layer_pretty_names')
    has_new_settings = hasattr(settings, 'kerasvis_layer_pretty_name_fn')
    if has_old_settings and not has_new_settings:
        print ('WARNING: Your settings.py and/or settings_local.py are out of date.'
               'kerasvis_layer_pretty_names has been replaced with kerasvis_layer_pretty_name_fn.'
               'Update your settings.py and/or settings_local.py (see documentation in'
               'setttings.py) to remove this warning.')
        return settings.kerasvis_layer_pretty_names.get(layer_name, layer_name)

    ret = layer_name
    if hasattr(settings, 'kerasvis_layer_pretty_name_fn'):
        ret = settings.kerasvis_layer_pretty_name_fn(ret)
    if ret != layer_name:
        print '  Prettified layer name: "{}" -> "{}"'.format(layer_name, ret)
    return ret
