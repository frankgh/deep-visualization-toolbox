...
class KerasVisAppState(object):
    '''State of KerasVis app.'''

    def __init__(self, net, settings, bindings, net_layer_info):
        self.lock = Lock()  # State is accessed in multiple threads
        self.settings = settings
        self.bindings = bindings
        self._layers = [layer.name for layer in net.layers]

        if hasattr(self.settings, 'kerasvis_filter_layers'):
            for name in self._layers:
                if self.settings.kerasvis_filter_layers(name):
                    print '  Layer filtered out by kerasvis_filter_layers: %s' % name
            self._layers = filter(lambda name: not self.settings.kerasvis_filter_layers(name), self._layers)
        self.net_layer_info = net_layer_info
        self.layer_boost_indiv_choices = self.settings.kerasvis_boost_indiv_choices  # 0-1, 0 is noop
        self.layer_boost_gamma_choices = self.settings.kerasvis_boost_gamma_choices  # 0-inf, 1 is noop
        self.keras_net_state = 'free'  # 'free', 'proc', or 'draw'
        self.extra_msg = ''
    ...
    def _reset_user_state(self):
        ...
        self.layer_boost_indiv_idx = self.settings.kerasvis_boost_indiv_default_idx
        self.layer_boost_indiv = self.layer_boost_indiv_choices[self.layer_boost_indiv_idx]
        self.layer_boost_gamma_idx = self.settings.kerasvis_boost_gamma_default_idx
    ...
    def handle_key(self, key):
        ...
        with self.lock:
            ...
            if tag == 'reset_state':
                ...
                self.move_selection('right', self.settings.kerasvis_fast_move_dist)
                ...
                if self.pattern_mode and not hasattr(self.settings, 'kerasvis_unit_jpg_dir'):
                    print 'Cannot switch to pattern mode; kerasvis_unit_jpg_dir not defined in settings.py.'
