...
from kerasvis_helper import crop_to_corner


class JPGVisLoadingThread(CodependentThread):
    '''Loads JPGs necessary for kerasvis_jpgvis pane in separate
        ...
        while not self.is_timed_out():
            with self.state.lock:
                ...
                if self.debug_level == 3:
                    print 'JPGVisLoadingThread.run: keras_net_state is: {}'.format(self.state.keras_net_state)
                    print 'JPGVisLoadingThread.run loop: next_frame: {}, keras_net_state: {}, back_enabled: {}'.format(
            ...
            # resize_shape = (None, data_shape[1]) if self.settings.kerasvis_jpgvis_stack_vert else (data_shape[0], None)
            # As a heuristic, instead just assume the three images are of the same shape.
            if self.settings.kerasvis_jpgvis_stack_vert:
            ...
            try:
                ...
                print '\nAttempted to load file %s but failed. To supress this warning, remove layer "%s" from settings.kerasvis_jpgvis_layers' % (
            ...
            if len(images) > 0:
                ...
                stack_axis = 0 if self.settings.kerasvis_jpgvis_stack_vert else 1
